...
ssl_selfs_signed_config.cert_reqs = ssl.CERT_NONE
# ssl_selfs_signed_config.ca_certs = os.path.join(current_dir, 'secrets/test.ca.pem')
ssl_selfs_signed_config.keyfile = os.path.join(current_dir, 'secrets/test.client.key')
ssl_selfs_signed_config.certfile = os.path.join(current_dir, 'secrets/test.client.crt')
...
class TestSSLConnection(TestCase):
    ...
    def test_create_selfs_signed(self):
        try:
            conn = Connection()
    ...
    def test_release_selfs_signed(self):
        try:
            conn = Connection()
            conn.open_SSL(host, port, 1000, ssl_selfs_signed_config)
            auth_result = conn.authenticate('root', 'nebula')
    ...
    def test_close_selfs_signed(self):
        conn = Connection()
        conn.open_SSL(host, port, 1000, ssl_selfs_signed_config)
        auth_result = conn.authenticate('root', 'nebula')
        assert auth_result.get_session_id() != 0
